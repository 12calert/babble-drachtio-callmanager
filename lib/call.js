...
class call {
  ...
  */
  ...
  async _runattendedxfer( req, res, replaces, replacesuri ) {
    ...
    /* RFC 3891 requires all three - and .match returns null rather than a short
       array when a tag is absent, so these must be checked before .length is
       read. Without the null tests this guard throws on exactly the malformed
       input it exists to reject, and the throw is swallowed by the catch in
       dialog.on( "refer" ) - leaving the transferor with no response at all. */
    if( !replaces || !totag || !fromtag ||
        3 > replaces.length || 3 > totag.length || 3 > fromtag.length ) {
